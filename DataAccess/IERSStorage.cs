...
public interface IERSStorage
{
    ...
    Ticket GetFromQueue();  // Will return the Ticket that is next in line of queue (FIFO)
    ...
    void NewTicket(Ticket t, string username);  // Will append the data from Ticket object with username as user reference to Tickets data

    // Data Checks
    bool CheckUsername(string username);    // Will return true/false depending on if username exists within LogIns data
    ...
    bool IsManager(string username);    // Will return true/false depending on if username exists within Managers data
